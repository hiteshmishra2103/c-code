...
struct node* insert(struct node *root, int key)
{
    ...
    if(head==NULL){//checking if tree is empty, if it is then point to new node
        head=new;
        return head;
    }
    ...
    while (root != NULL)
    {
        prev = root; // prev pointer will hold the value of previous node
        if (key == root->data)
        {
            printf("Cannot insert node of same data!\n");
            return head;
        }
        else if (key < root->data)
        {
